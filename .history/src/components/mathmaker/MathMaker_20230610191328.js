...
const MathMaker = () => {
  ...
  const handleScheduleMeeting = () => {
    ...
    if (selectedUser) {
      // Get input values (zoom link, time)
      ...
      // Append the schedule object to the selectedUser document's "schedule" field
      db.collection('users').doc(selectedUser.user.username)
        .update({
          schedule: firebase.firestore.FieldValue.arrayUnion(scheduleData),
        })
        .then(() => {
          console.log("Schedule data added to Firestore successfully!");
          // Optionally, you can perform additional actions after the data is successfully added
        })
        .catch((error) => {
